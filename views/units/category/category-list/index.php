...
<body>
            ...
            <div class="col-xl-12">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">
                    Kategorilar Listesi
                    <a href="<?php echo $this->pathHtml; ?>category/new-category" class="btn btn-primary btn-sm float-right"> <span class="fa fa-plus"></span> Yeni Kategori Ekle</a>
                  </h4>
                  <p class="card-subtitle mb-4">Kategorilarınızın listesine buradan ulaşabilirsiniz</p>
                  <div class="table-responsive">
                    <form class="form-inline frmTableSearch" model="category" action="" method="post">
                      <div class="form-group">
                        <label for="txtCategoryName">Kategori Adı : </label>
                        <input type="text" class="form-control" name="txtCategoryName" id="txtCategoryName" value="">
                      </div>
                      <div class="form-group">
                        <button type="submit" class="btn btn-primary btn-sm" name="button">Ara</button>
                      </div>
                    </form>
                    <table class="table mb-0" id="category-list" model="category" is-override="false" page-number="1" item-per-page="1"  >
                      ...
                      <tbody>
                        ...
                        // for ($i=0; $i < count($this->categorys); $i++) {
                          // echo '<tr id='.$this->categorys[$i]["category_id"].'">';
                          // echo '<td>'.$this->categorys[$i]["category_id"].'</td>';
                          // echo '<td>'.$this->categorys[$i]["category_name"].'</td>';
                          // echo '<td>'.$this->categorys[$i]["category_phone_number"].'</td>';
                          // echo '<td>'.$this->categorys[$i]["category_email"].'</td>';
                          // echo '<td>'.$this->categorys[$i]["category_username"].'</td>';
                          // echo '<td>';
  ...
  <script src="<?php echo $this->pathHtml; ?>views/units/category/category-list/category-list.js"></script>
