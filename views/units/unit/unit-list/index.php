...
<body>
            ...
            <div class="col-xl-12">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">
                    Birimlar Listesi
                    <a href="<?php echo $this->pathHtml; ?>unit/new-unit" class="btn btn-primary btn-sm float-right"> <span class="fa fa-plus"></span> Yeni Birim Ekle</a>
                  </h4>
                  <p class="card-subtitle mb-4">Birimlarınızın listesine buradan ulaşabilirsiniz</p>
                  <div class="table-responsive">
                    <form class="form-inline frmTableSearch" model="unit" action="" method="post">
                      <div class="form-group">
                        <label for="txtUnitName">Birim Adı : </label>
                        <input type="text" class="form-control" name="txtUnitName" id="txtUnitName" value="">
                      </div>
                      <div class="form-group">
                        <button type="submit" class="btn btn-primary btn-sm" name="button">Ara</button>
                      </div>
                    </form>
                    <table class="table mb-0" id="unit-list" model="unit" is-override="false" page-number="1" item-per-page="1" >
                      ...
                      <tbody>
                        ...
                        // for ($i=0; $i < count($this->units); $i++) {
                          // echo '<tr id='.$this->units[$i]["unit_id"].'">';
                          // echo '<td>'.$this->units[$i]["unit_id"].'</td>';
                          // echo '<td>'.$this->units[$i]["unit_name"].'</td>';
                          // echo '<td>'.$this->units[$i]["unit_phone_number"].'</td>';
                          // echo '<td>'.$this->units[$i]["unit_email"].'</td>';
                          // echo '<td>'.$this->units[$i]["unit_username"].'</td>';
                          // echo '<td>';
  ...
  <script src="<?php echo $this->pathHtml; ?>views/units/unit/unit-list/unit-list.js"></script>
