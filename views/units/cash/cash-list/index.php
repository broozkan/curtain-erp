...
<body>
            ...
            <div class="col-xl-12">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">
                    Kasalar Listesi
                    <a href="<?php echo $this->pathHtml; ?>cash/new-cash" class="btn btn-primary btn-sm float-right"> <span class="fa fa-plus"></span> Yeni Kasa Ekle</a>
                  </h4>
                  <p class="card-subtitle mb-4">Kasalarınızın listesine buradan ulaşabilirsiniz</p>
                  <div class="table-responsive">
                    <form class="form-inline frmTableSearch" model="cash" action="" method="post">
                      <div class="form-group">
                        <label for="txtCashName">Kasa Adı : </label>
                        <input type="text" class="form-control" name="txtCashName" id="txtCashName" value="">
                      </div>
                      <div class="form-group">
                        <button type="submit" class="btn btn-primary btn-sm" name="button">Ara</button>
                      </div>
                    </form>
                    <table class="table mb-0" id="cash-list" model="cash" is-override="false" page-number="1" item-per-page="1" >
                      ...
                      <tbody>
                        ...
                        <?php
                        // for ($i=0; $i < count($this->cashs); $i++) {
                          // echo '<tr id='.$this->cashs[$i]["cash_id"].'">';
                          // echo '<td>'.$this->cashs[$i]["cash_id"].'</td>';
                          // echo '<td>'.$this->cashs[$i]["cash_name"].'</td>';
                          // echo '<td>'.$this->cashs[$i]["cash_phone_number"].'</td>';
                          // echo '<td>'.$this->cashs[$i]["cash_email"].'</td>';
                          // echo '<td>'.$this->cashs[$i]["cash_username"].'</td>';
                          // echo '<td>';
  ...
  <script src="<?php echo $this->pathHtml; ?>views/units/cash/cash-list/cash-list.js"></script>
