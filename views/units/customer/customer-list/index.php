...
<body>
            ...
            <div class="col-xl-12">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">
                    Müşteriler Listesi
                    <a href="<?php echo $this->pathHtml; ?>customer/new-customer" class="btn btn-primary btn-sm float-right"> <span class="fa fa-plus"></span> Yeni Müşteri Ekle</a>
                  </h4>
                  <p class="card-subtitle mb-4">Müşterilerınızın listesine buradan ulaşabilirsiniz</p>
                  <div class="table-responsive">
                    <form class="form-inline frmTableSearch" model="customer" action="" method="post">
                      <div class="form-group">
                        <label for="txtCustomerName">Müşteri Adı : </label>
                        <input type="text" class="form-control" name="txtCustomerName" id="txtCustomerName" value="">
                      </div>
                      <div class="form-group">
                        <button type="submit" class="btn btn-primary btn-sm" name="button">Ara</button>
                      </div>
                    </form>
                    <table class="table mb-0" id="customer-list" model="customer" is-override="false" page-number="1" item-per-page="1">
                      <thead class="thead-light">
                        <tr>
                          <th>#</th>
                          <th>Müşteri Adı</th>
                          <th>Müşteri Adresi</th>
                          <th>Müşteri E-Posta Adresi</th>
                      ...
                      <tbody>
                        ...
                        // echo '<tr id='.$this->customers[$i]["customer_id"].'">';
                        // echo '<td>'.$this->customers[$i]["customer_id"].'</td>';
                        // echo '<td>'.$this->customers[$i]["customer_name"].'</td>';
                        // echo '<td>'.$this->customers[$i]["customer_phone_number"].'</td>';
                        // echo '<td>'.$this->customers[$i]["customer_email"].'</td>';
                        // echo '<td>'.$this->customers[$i]["customer_username"].'</td>';
                        // echo '<td>';
  ...
  <script src="<?php echo $this->pathHtml; ?>views/units/customer/customer-list/customer-list.js"></script>
