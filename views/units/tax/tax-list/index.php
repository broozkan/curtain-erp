...
<body>
            ...
            <div class="col-xl-12">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">
                    Vergiler Listesi
                    <a href="<?php echo $this->pathHtml; ?>tax/new-tax" class="btn btn-primary btn-sm float-right"> <span class="fa fa-plus"></span> Yeni Vergi Ekle</a>
                  </h4>
                  <p class="card-subtitle mb-4">Vergilerınızın listesine buradan ulaşabilirsiniz</p>
                  <div class="table-responsive">
                    <form class="form-inline frmTableSearch" model="tax" action="" method="post">
                      <div class="form-group">
                        <label for="txtTaxName">Vergi Adı : </label>
                        <input type="text" class="form-control" name="txtTaxName" id="txtTaxName" value="">
                      </div>
                      <div class="form-group">
                        <button type="submit" class="btn btn-primary btn-sm" name="button">Ara</button>
                      </div>
                    </form>
                    <table class="table mb-0" id="tax-list" model="tax" is-override="false" page-number="1" item-per-page="1" >
                      ...
                      <tbody>
                        ...
                        <?php
                        // for ($i=0; $i < count($this->taxs); $i++) {
                        // echo '<tr id='.$this->taxs[$i]["tax_id"].'">';
                        // echo '<td>'.$this->taxs[$i]["tax_id"].'</td>';
                        // echo '<td>'.$this->taxs[$i]["tax_name"].'</td>';
                        // echo '<td>'.$this->taxs[$i]["tax_phone_number"].'</td>';
                        // echo '<td>'.$this->taxs[$i]["tax_email"].'</td>';
                        // echo '<td>'.$this->taxs[$i]["tax_username"].'</td>';
                        // echo '<td>';
  ...
  <script src="<?php echo $this->pathHtml; ?>views/units/tax/tax-list/tax-list.js"></script>
